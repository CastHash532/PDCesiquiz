...
public class SupprimerNotionController {
    ...
    public void initialize(ESIQuiz quiz, Formateur c)
    {
        ...
        ObservableList<Notion> list = FXCollections.observableArrayList();

        while(iter.hasNext()) {
            list.add(iter.next());
        }
        notions.setItems(list);
    }
    ...
    @FXML
    private ChoiceBox<Notion> notions;
    ...
    void Supprimer(ActionEvent event) {
        if(notions.getValue() != null)
        {
            current.supprimerNotion(notions.getValue());
        }
